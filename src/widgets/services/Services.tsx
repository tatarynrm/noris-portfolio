                ...
                <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-10">
                    {servicesData.map((service, i) => (
                        <div
                            ...
                            className={`service-card transform-gpu will-change-transform relative group p-10 rounded-3xl backdrop-blur-xl bg-white/60 dark:bg-black/40 border border-gray-200/50 dark:border-white/10 shadow-card dark:shadow-[0_15px_35px_rgba(0,0,0,0.3)] hover:shadow-hover dark:hover:shadow-[0_15px_40px_rgba(59,130,246,0.2)] transition-colors duration-500 ${service.border} h-full flex flex-col`}
                        >
                            {/* Inner ambient glow */}
                            ...
                            <div className="mt-auto pt-6 border-t border-gray-100 dark:border-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <Link
                                    href={`/services/${service.slug}`}
                                    className="inline-flex items-center text-blue-500 font-bold hover:text-blue-400 transition-colors"
                                >
                                    {t("deep_dive")} <span className="ml-2">→</span>
                                </Link>
                            </div>
                        </div>
                    ))}
